/***
 * The only difference between this and the base module is the initializer.
 */
 ...
 */
    // Fallback defaults for new vars added between major releases.
    // v1.0.5
    var ed11yMessageQAMayBeHeader = (typeof ed11yMessageQAMayBeHeader === 'undefined') ? "<div class='ed11y-tip-heading'>Manual check needed: Possible heading</div><p>This whole paragraph is bold. If this is for emphasis, this is fine. If it is acting as a heading or subheading, however, please format it as such so that it can become part of the page outline.</p>" : ed11yMessageQAMayBeHeader;
    // v1.0.9
    ed11yIgnoreLinkStrings = (typeof ed11yIgnoreLinkStrings === 'undefined') ? "" : ed11yIgnoreLinkStrings;
    var ed11yOnce;
  ...
  Drupal.behaviors.editoria11y = {
    attach: function (context, settings) {
      ...
      if (context === document && !ed11yOnce) {
        ...
        // Override editoria11y-prefs.js with config from the database.
        ed11yCheckRoot = drupalSettings.editoria11y.content_root ? drupalSettings.editoria11y.content_root : ed11yCheckRoot;
        if (drupalSettings.editoria11y.no_load && drupalSettings.editoria11y.no_load.trim().length > 0) {
          ed11yNoRun += ", " + drupalSettings.editoria11y.no_load;
        }
        if (drupalSettings.editoria11y.ignore_containers && drupalSettings.editoria11y.ignore_containers.trim().length > 0) {
          ed11yContainerIgnore += ", " + drupalSettings.editoria11y.ignore_containers;
        }
        if (drupalSettings.editoria11y.download_links && drupalSettings.editoria11y.download_links.length > 0) {
          ed11yDownloadLinks += ", " + drupalSettings.editoria11y.download_links;
        }

        // If assertiveness is smart we open the panel on errors for one minute.
        ed11yAlertMode = drupalSettings.editoria11y.assertiveness ? drupalSettings.editoria11y.assertiveness : ed11yAlertMode;
        const now = new Date();
        if (ed11yAlertMode === "smart" && (now / 1000) - drupalSettings.editoria11y.changed < 60) {
          ed11yAlertMode = "assertive";
        }

        // These add to rather than replace defaults so that multisite
        // environments can have an overridden editoria11y-prefs.js defaults
        // file.
        if (drupalSettings.editoria11y.embedded_content_warning && drupalSettings.editoria11y.embedded_content_warning.trim().length > 0) {
          if (ed11yEmbeddedContentWarning.length === 0) {
            ed11yEmbeddedContentWarning = drupalSettings.editoria11y.embedded_content_warning;
          }
          else {
            ed11yEmbeddedContentWarning += ", " + drupalSettings.editoria11y.embedded_content_warning;
          }
        ...
        if (drupalSettings.editoria11y.allow_overflow && drupalSettings.editoria11y.allow_overflow.trim().length > 0) {
          if (ed11yAllowOverflow.length === 0) {
            ed11yAllowOverflow = drupalSettings.editoria11y.allow_overflow;
          } else {
            ed11yAllowOverflow += ", " + drupalSettings.editoria11y.allow_overflow;
          }
        }

        if (drupalSettings.editoria11y.hidden_handlers && drupalSettings.editoria11y.hidden_handlers.trim().length > 0) {
          if (ed11yHiddenHandlers.length === 0) {
            ed11yHiddenHandlers = drupalSettings.editoria11y.hidden_handlers;
          }
          else {
            ed11yHiddenHandlers += ", " + drupalSettings.editoria11y.hidden_handlers;
          }
        }
        if (drupalSettings.editoria11y.ignore_link_strings && drupalSettings.editoria11y.ignore_link_strings.trim().length > 0) {
          ed11yIgnoreLinkStrings = new RegExp(drupalSettings.editoria11y.ignore_link_strings, "g");
        }
        ...
        /***
         * Paste below here the Ed11y base library, minus jQuery start wrapper.
         * @constructor
         */
        ...
        // Drupal specific close tags follow.
      }
    }
  };
})(jQuery);
